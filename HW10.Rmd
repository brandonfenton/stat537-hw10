...
```{r p1_a, echo=F, fig.height=10, fig.width=10} 
set.seed(888)
...
gpa.tree <- rpart(FYGPA ~ ., data = satGPA, cp = 1e-09, minbucket = 4)
...
A call to __plotcp()__ on one tree fit suggests that a tree size of 5 should be used by the 1-SE rule, and a tree size of 11 should be used by the min-CV rule.  Fitting 100 cross-validated recursive partitions provides further support for these sizes:
...
gpa.ose <-rpart(FYGPA~.,data=satGPA,cp=1.5e-02, minbucket=4)
gpa.mcv <-rpart(FYGPA~.,data=satGPA,cp=5.9734e-03, minbucket=4)
```
